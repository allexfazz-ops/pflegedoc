 ...
 * `req` e opțional: dacă e dat, un depășit generează un eveniment de securitate
 * (`rate_limit_hit`) cu doar prefixul bucket-ului (fără IP/uuid brut).
 */
export async function enforceRateLimit(res, bucket, limit, windowSeconds, req) {
    ...
    if (!r.allowed) {
        ...
        if (req) {
            const label = String(bucket).split(":").slice(0, 2).join(":"); // ex: "login:ip"
            securityEvent("rate_limit_hit", req, { outcome: "429", detail: label });
        }
        return true;
    }
    return false;
