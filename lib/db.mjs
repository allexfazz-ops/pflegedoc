 ...
 * - `getPool()`    : Pool (protocol PG peste WebSocket) pentru migrări / tranzacții
 * - `ensureSchema()`: aplică db/schema.sql o singură dată per proces (idempotent)
 *
 * Variabilele de conexiune sunt injectate automat de integrarea Neon din Vercel
 * (DATABASE_URL etc.). NU sunt niciodată expuse frontend-ului.
 */
...
export const sql = CONNECTION_STRING ? neon(CONNECTION_STRING) : missing;

/** Pool pentru migrări / multi-statement / tranzacții. Închide-l cu `pool.end()`. */
export function getPool() {
    if (!CONNECTION_STRING) missing();
    return new Pool({ connectionString: CONNECTION_STRING });
}

export const hasDatabase = () => Boolean(CONNECTION_STRING);
